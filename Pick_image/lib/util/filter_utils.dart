...
class FilterUtils{
  static final Map<String, List<int>> _catchFilter = {};

  static void clearCache() => _catchFilter.clear();

  static void savveCachedFilter(Filter filter, List<int> imageBytes){
    if (filter == null) return;

    _catchFilter[filter.name] = imageBytes;
  }

  static List<int> getCachedFilter(Filter filter){
    if(filter == null) return null;
    return _catchFilter[filter.name];
  }

  static Future<List<int>> applyFilter(img.Image image, Filter filter){
    if(filter == null) throw Exception('Filter not set');

    return compute(_applyFilterInternal,
        <String, dynamic>{
          'filter' : filter,
          'image' : image,
          'width' : image.width,
