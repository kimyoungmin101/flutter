...
import 'package:p1_project/home/components/constants.dart';
import 'package:p1_project/home/components/product_delete.dart';
import 'package:p1_project/home/components/product_modify.dart';
...
class Product2_Value extends StatefulWidget {
  static String routeName = "/Product2_Value";

  final int id;
  final String token;
  final String img_path;

  const Product2_Value({Key key, this.id, this.token, this.img_path})
      : super(key: key);

  @override
  State<Product2_Value> createState() =>
      _Product2_ValueState(id, token, img_path);
}

class _Product2_ValueState extends State<Product2_Value> {
  int id;
  String token;
  ...
  _Product2_ValueState(int id, String token, String img_path) {
    this.id = id;
    this.token = token;
  ...
  Future<Product2_Value> product_get(product_id, token) async {
    final http.Response response = await http.get(
        Uri.parse('http://neuro.iptime.org:3080/product/$product_id'),
        headers: {
  ...
  void choiceAction(String choice) {
    if (choice == Constants.Modify) {
      ...
      showAlertDialog(context, id, token);
    }
  }
}

showAlertDialog(BuildContext context, id, token) {
  // set up the buttons
  ...
  Widget continueButton = FlatButton(
    child: Text("삭제"),
    onPressed: () {
      Future<Product_Delete> deleteProduct(id, token) async {
        final http.Response response = await http.delete(
            Uri.parse('http://neuro.iptime.org:3080/product/$id'),
            headers: {
              'Content-Type': 'application/json',
              'Accept': 'application/json',
              'Authorization': 'Bearer $token',
